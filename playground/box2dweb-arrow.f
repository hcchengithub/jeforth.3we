	...
	\ Box2dWeb.js 採用如上 <tag> 的方式 include 進來，很自然，有如 jQuery 從 index.html 裡
	\ 加載，等效。但是 arrow.js 卻不很適合這樣 include, 因為要在裡面放 jsc Break-Point 
	\ "if(kvm.debug){kvm.jsc.prompt='msg';eval(kvm.jsc.xt)}" 以便 debg 而如果用 <tag> 的方
	\ 式，就會定義在 jeforth 之外，因而享受不到 jsc 的好處。例如 execute('(see)') 這樣的小工
	\ 具都不能用。連 print() 也變成 window.print()，趣味盡失。所以原:
	\ <h> <script type="text/javascript" src="playground/arrow.js"></script></h> constant arrow.js // ( -- obj ) The arrow.js script element
	\ 這一行要改成下列方法，

	char arrow.js readTextFileAuto <text>
		init();
	</text> + js: eval(pop())
