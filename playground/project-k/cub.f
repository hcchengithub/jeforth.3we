...
<source>
	...
	code stop       ntib=tib.length;stop=true end-code // ( -- ) Stop the TIB loop
	...
	code code       push(nexttoken()); // name of the word
					...
					execute(words.root[1]); // execute the old version 'code'.
					end-code
	...
	code </selftest> ( "selftest" -- ) \ Save the self-test statements to <selftest>.buffer. interpret-only
					...
					<selftest>
						...
						***   : start of a selftest section with description. Stack must be empty.
