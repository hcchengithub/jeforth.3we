	...
	***	上面這兩個研究得出心得：要平滑 vx vy 要小，這條件下要飛得快 interval nap 就要小，考驗電腦速度。
	...
	[ ] 有時會亂飛，有時會漸趨平靜，怎麼可能？多出來的動能哪來的，損失到哪去了？
		...
		==> 觀察下面這組實驗設計，當 D 與 y軸 夾角越小時，誤差越大。猜想因為這夾角越小越偏向「擦撞」，
			dy 幾等於 distance 而 dx 很小很小，這應該不利於 atan2(dy,dx) 所算出來的角度。
			...
			this.collide = function(){
				for (var i = id - 1; i >= 1; i--) {  // 只管自己 id 以下兩兩之間的 collision
					...
					if (distance < minDist) {   // 撞進球體裡面了
						...
						// fa = (a.vx,a.vy)(.)(ex,ey) = a.vx*ex + a.vy*ey
						var fax = (a.vx*ex + a.vy*ey)*ex; // 本球向量 (a.vx,a.vy) 在 (ex,ey) 上的投影
						var fay = (a.vx*ex + a.vy*ey)*ey;
						// 本球動量分走之後剩下的是 (a.vx,a.vy)-(fax,fay)
						var rax = a.vx-fax; // (rax, ray) 是本球未考慮反彈力的分量
						var ray = a.vy-fay;
						// fb 則是 the other ball, 另一球在反彈力方向上的投影
						var fbx = (b.vx*ex + b.vy*ey)*ex; // 他球向量 (b.vx,b.vy) 在 (ex,ey) 上的投影
						var fby = (b.vx*ex + b.vy*ey)*ey;
						// 他球動量分走之後剩下的是 (b.vx,b.vy)-(fbx,fby)
						var rbx = b.vx-fbx; // (r1x, r1y) 是他球未考慮反彈力的分量
						var rby = b.vy-fby;

						// 到底誰撞誰？應該是對稱平等的。做用力與反作用力 (fx,fy) 分別對雙方各用一次看似重複，其實
						// 因兩者大小相同、方向相反，因此總動量不變，很合理。
						
						// 本球最後向量 (avx,avy) = (rax,ray)-(fax,fay)+(fbx,fby) 自己的作用給別人，別人的作用給自己，交換作用力。
						// 他球最後向量 (bvx,bvy) = (rbx,rby)-(fbx,fby)+(fax,fay)
						avx = rax + fbx; // 本球最終向量
						avy = ray + fby;  
						bvx = rbx + fax; // 他球最終向量
						bvy = rby + fay;
						// avx = rax + fax + fbx; // 本球最終向量
						// avy = ray + fay + fby;  
						// bvx = rbx + fbx + fax; // 他球最終向量
						// bvy = rby + fby + fay;
						// 動量
