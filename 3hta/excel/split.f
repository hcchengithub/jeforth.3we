...
s" split.f" source-code-header
	...
	\ hcchen5600 2014/12/11 17:14:04 出 bug!!
	\ 靠！ 跑了半天，每張 ME.xls SW.xls 裡都只有 title rows 沒有員工 rows.
	\ [x] 先斷在 split-groups-of-rows-to-excel-files 來看看當時的情形。。。
	\ 原因揭曉：原來的 excel 的 title rows 中有「合併儲存格」，造成 bottom 算錯了，當
	\ rows.copy 之後因為算錯了 bottom 而要往有「合併儲存格」的 title sheet.paste(rows)
	\ 時就出錯了。程式不改，改原始 excel 檔，不要讓 bottom 算錯即可。
	\ 抓出這個問題很容易，在 "分發總表到各部門的worksheet" 裡設一設斷點，觀察幾下當時
	\ 的 variables 就知道了。
