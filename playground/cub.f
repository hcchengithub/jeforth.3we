...
\ 用 jeforth.3we 當作開發環境,打個 marker 以便重複驗證程式。
...
\ 、keyboard 這些東西，但它會 call vm.panic() 去處理 error。
	...
	k :: prompt="project-k-cub>"
		 ...
		 </text> <js> pop().replace(/^[\t ]*/gm,"")</jsV> \ remove leading Tab's and spaces
	k :: jsc={} \ JavaScript console
	k :: jsc.help=pop() 
		 <text>
			(function(){
				var _cmd_ = "";
...
\ 用個辦法來把 source code 包成一個 string		
	...
	: <source> ( <text> -- "text" ) \ Get multiple-line string
		char </source> word ; immediate
	: </source> ( "text" -- ... ) \ Delimiter of <text>
		compiling if literal then ; immediate
		/// Usage: <source> word of multiple lines </source>
	
<source>
	code //         last().help = nexttoken('\n|\r'); end-code
					// ( <comment> -- ) Give help message to the new word.
	code stop       ntib=tib.length;stop=true;debugger; end-code // ( -- ) Stop the TIB loop
