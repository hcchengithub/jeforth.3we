		...
		function Ball(ID,X,Y,RADIUS,COLOR){with(this){
			...
			this.collide = function(){
				for (var i = id - 1; i >= 1; i--) {  // 只管自己 id 以後兩兩之間的 collision
					...
					var dx = g.balls[i].x - x;  
					var dy = g.balls[i].y - y;  
					var distance = Math.sqrt(dx*dx + dy*dy);  // 碰撞時的球心距離，有凹陷，所以可小於 minDist
					var minDist = g.balls[i].radius + radius;  // 緊貼兩 ball 的球心距離。
					if (distance < minDist) {   // 撞上了！ 當兩球相撞時，總動量不變。
						...
						var ax = (targetX - g.balls[i].x) * g.spring;  // 另一球當在的位置與目前互相撞進球體之內的差距 乘上 彈性係數。方向是另一球該修正的方向。
						var ay = (targetY - g.balls[i].y) * g.spring;
